--[[
    ================================================================
    NEXZAN KEY SYSTEM - ELEGANT & COMPACT
    Mirip desain gambar referensi (dark modern UI)
    Drag-able, kecil (compact untuk mobil), rapi & enak dipandang
    Mode: Single (auto-copy link) / Multi (dropdown pilih link)
    ================================================================
    Author: Nexzan Hub UI Library
    Untuk digunakan sebagai library / loadstring
]]
...
local function ShowToast(parent, title, msg, duration, color)
    ...
    color = color or Color3.fromRGB(124, 58, 237)
    ...
    stroke.Color = color; stroke.Thickness = 1.2; stroke.Transparency = 0.4
    ...
    local tl = Instance.new("TextLabel", textFrame)
    tl.Size = UDim2.new(1, 0, 0, 14)
    tl.BackgroundTransparency = 1
    tl.Text = title; tl.TextColor3 = Color3.fromRGB(240, 240, 245)
    tl.Font = Enum.Font.GothamBold; tl.TextSize = 11; tl.TextXAlignment = Enum.TextXAlignment.Left
    local ml = Instance.new("TextLabel", textFrame)
    ml.Size = UDim2.new(1, 0, 0, 12); ml.Position = UDim2.new(0, 0, 0, 14)
    ml.BackgroundTransparency = 1
    ml.Text = msg; ml.TextColor3 = Color3.fromRGB(170, 170, 185)
...
function NexzanKeyUI:Create(cfg)
    ...
    local URLs       = cfg.URLs or cfg.Links or {
        ["Discord"] = "https://discord.gg/nexzanhub",
        ["Work.ink"] = "https://work.ink/nexzan",
        ["Pastebin"] = "https://pastebin.com/raw/nexzan"
    }
    local Accent     = cfg.Accent or Color3.fromRGB(139, 92, 246) -- ungu modern
    local Size       = cfg.Size or "Compact"        -- Compact (kecil) atau Normal
    ...
    -- Warna palet modern (mirip gambar)
    local c = {
    ...
    -- Ukuran (Compact = kecil untuk mobil)
                    ...
                    if content and content ~= "" then
                        if type(Keys) == "table" then
                            for _, k in ipairs(Keys) do
                                if tostring(k) == content then savedKey = content; break end
                            end
    ...
    if getgenv and getgenv().NEXZAN_KEY then
        ...
        if type(Keys) == "table" then
            for _, k in ipairs(Keys) do
                if tostring(k) == ext then savedKey = ext; break end
            end
        elseif type(Keys) == "string" and Keys == ext then
            savedKey = ext
        end
        if savedKey then
            pcall(OnSuccess, savedKey)
    ...
    -- ===== MAIN CARD (mirip gambar - modern dark) =====
    ...
    BorderStroke.Color = c.accent; BorderStroke.Thickness = 1.2
    BorderStroke.Transparency = 0.55

    -- ===== HEADER (bisa digeser / drag) =====
    local Header = Instance.new("Frame", Main)
    Header.Name = "Header"
    Header.Size = UDim2.new(1, 0, 0, isCompact and 50 or 58)
    Header.BackgroundColor3 = c.surface
    Header.BorderSizePixel = 0
    Header.ZIndex = 5
    Instance.new("UICorner", Header).CornerRadius = UDim.new(0, 8)
    -- Hanya bagian atas yang rounded
    -- Untuk kesederhanaan, kita biarkan rounded semua tapi tetap bagus

    -- Gradient line bawah header
    ...
    LockIcon.Text = "🔐"
    LockIcon.TextColor3 = c.accent
    LockIcon.Font = Enum.Font.SourceSansBold
    LockIcon.TextSize = isCompact and 16 or 20

    -- Title & SubTitle
    local TitleLbl = Instance.new("TextLabel", Header)
    TitleLbl.Name = "Title"
    TitleLbl.Size = UDim2.new(1, -(isCompact and 80 or 100), 0, 22)
    TitleLbl.Position = UDim2.new(0, isCompact and 52 or 62, 0.5, -14)
    TitleLbl.BackgroundTransparency = 1
    TitleLbl.Text = Title
    TitleLbl.TextColor3 = c.text
    TitleLbl.Font = Enum.Font.GothamBold
    TitleLbl.TextSize = isCompact and 15 or 18
    TitleLbl.TextXAlignment = Enum.TextXAlignment.Left

    local SubLbl = Instance.new("TextLabel", Header)
    SubLbl.Name = "SubTitle"
    SubLbl.Size = UDim2.new(1, -(isCompact and 80 or 100), 0, 16)
    ...
    SubLbl.TextSize = isCompact and 11 or 13
    ...
    Content.Size = UDim2.new(1, -(isCompact and 24 or 30), 1, -(isCompact and 58 or 66))
    Content.Position = UDim2.new(0, isCompact and 12 or 15, 0, isCompact and 54 or 62)
    Content.BackgroundTransparency = 1

    local ContentLayout = Instance.new("UIListLayout", Content)
    ContentLayout.SortOrder = Enum.SortOrder.LayoutOrder
    ContentLayout.Padding = UDim.new(0, isCompact and 6 or 8)
    ...
    GKBInner.Name = "Inner"
    GKBInner.Size = UDim2.new(1, -16, 1, 0)
    GKBInner.Position = UDim2.new(0, 8, 0, 0)
    GKBInner.BackgroundTransparency = 1

    local GKBLayout = Instance.new("UIListLayout", GKBInner)
    GKBLayout.FillDirection = Enum.FillDirection.Horizontal
    GKBLayout.VerticalAlignment = Enum.VerticalAlignment.Center
    GKBLayout.Padding = UDim.new(0, 8)

    -- Icon kunci
    local GKBIcon = Instance.new("TextLabel", GKBInner)
    GKBIcon.Size = UDim2.fromOffset(isCompact and 20 or 24, isCompact and 20 or 24)
    GKBIcon.BackgroundTransparency = 1
    GKBIcon.Text = "🔑"
    GKBIcon.TextColor3 = c.accent
    GKBIcon.Font = Enum.Font.GothamBold
    GKBIcon.TextSize = isCompact and 15 or 18

    -- Frame teks
    local GKBTextFrame = Instance.new("Frame", GKBInner)
    GKBTextFrame.Size = UDim2.new(1, -(isCompact and 44 or 56), 1, 0)
    GKBTextFrame.BackgroundTransparency = 1

    local GKBTitle = Instance.new("TextLabel", GKBTextFrame)
    GKBTitle.Size = UDim2.new(1, 0, 0, 20)
    GKBTitle.BackgroundTransparency = 1
    GKBTitle.Text = "GET YOUR KEY"
    GKBTitle.TextColor3 = c.text
    GKBTitle.Font = Enum.Font.GothamBold
    GKBTitle.TextSize = isCompact and 13 or 15
    GKBTitle.TextXAlignment = Enum.TextXAlignment.Left

    local GKBSub = Instance.new("TextLabel", GKBTextFrame)
    GKBSub.Name = "SubDisplay"
    GKBSub.Size = UDim2.new(1, 0, 0, 18)
    GKBSub.Position = UDim2.new(0, 0, 0, 18)
    GKBSub.BackgroundTransparency = 1
    GKBSub.Text = ""
    GKBSub.TextColor3 = c.textMuted
    GKBSub.Font = Enum.Font.SourceSans
    GKBSub.TextSize = isCompact and 9 or 11
    GKBSub.TextXAlignment = Enum.TextXAlignment.Left

    -- Arrow kanan
    local GKBArrow = Instance.new("TextLabel", GKBInner)
    GKBArrow.Size = UDim2.fromOffset(22, 22)
    GKBArrow.BackgroundTransparency = 1
    GKBArrow.Text = "→"
    GKBArrow.TextColor3 = c.textDim
    GKBArrow.Font = Enum.Font.GothamBold
    GKBArrow.TextSize = 16

    -- Set teks sub sesuai mode
    if Mode == "Single" then
    ...
    GetKeyBtn.MouseEnter:Connect(function()
        ...
        GKBArrow.Position = UDim2.fromOffset(26, 0)
    end)
    GetKeyBtn.MouseLeave:Connect(function()
        ...
        GKBArrow.Position = UDim2.fromOffset(22, 0)
    end)

    -- ===== DROPDOWN MULTI (tersembunyi default) =====
    local DropHolder = Instance.new("Frame", Content)
    DropHolder.Name = "DropHolder"
    DropHolder.LayoutOrder = 2
    ...
    local InPad = Instance.new("UIPadding", InputFrame)
    InPad.PaddingLeft = UDim.new(0, isCompact and 12 or 14)

    local TextBox = Instance.new("TextBox", InputFrame)
    TextBox.Size = UDim2.new(1, -(isCompact and 12 or 14), 1, 0)
    TextBox.BackgroundTransparency = 1
    TextBox.PlaceholderText = "Masukkan key disini..."
    TextBox.Text = ""
    TextBox.TextColor3 = c.text
    TextBox.PlaceholderColor3 = c.textMuted
    TextBox.Font = Enum.Font.SourceSans
    TextBox.TextSize = isCompact and 13 or 15
    TextBox.ClearTextOnFocus = false
    TextBox.TextXAlignment = Enum.TextXAlignment.Left

    -- Counter karakter kecil
    local CharCount = Instance.new("TextLabel", InputFrame)
    CharCount.Size = UDim2.fromOffset(50, 14)
    CharCount.Position = UDim2.new(1, -54, 1, -2)
    CharCount.BackgroundTransparency = 1
    CharCount.Text = "0/50"
    CharCount.TextColor3 = c.textMuted
    ...
    ResetBtn.BackgroundColor3 = c.surface2
    ResetBtn.Text = "↺"
    ...
    local function SetStatus(text, color)
        StatusLbl.Text = text
        StatusLbl.TextColor3 = color or c.error
    end
        ...
        elseif Mode == "Multi" then
            ...
            if visible then
                ...
                for _, btn in ipairs(items) do
                    totalH = totalH + (btn.AbsoluteSize.Y or 36) + 5
                end
                ...
                GKBSub.Text = "▼  Tap untuk pilih link"
                TweenService:Create(DropHolder, TweenInfo.new(0.18), {
    ...
    -- ===== BANGUN DROPDOWN MULTI =====
    if Mode == "Multi" then
        local linksData = {}
        if URLs and type(URLs) == "table" then
            local isDict = false
            for k, _ in pairs(URLs) do
                if type(k) == "string" then isDict = true; break end
            end
            if isDict then
                for name, url in pairs(URLs) do
                    table.insert(linksData, { Name = name, URL = url })
                end
            else
                for i, url in ipairs(URLs) do
                    table.insert(linksData, { Name = "Link " .. i, URL = url })
                end
            end
        else
        ...
        for i, linkData in ipairs(linksData) do
            ...
            itLayout.VerticalAlignment = Enum.VerticalAlignment.Center
            itLayout.Padding = UDim.new(0, 8)
            local itPad = Instance.new("UIPadding", item)
            itPad.PaddingLeft = UDim.new(0, 10)

            -- Icon
            local itIcon = Instance.new("TextLabel", item)
            itIcon.Size = UDim2.fromOffset(isCompact and 18 or 22, isCompact and 18 or 22)
            itIcon.BackgroundTransparency = 1
            itIcon.Text = "🔗"
            itIcon.TextColor3 = c.accent
            itIcon.Font = Enum.Font.SourceSansBold
            itIcon.TextSize = isCompact and 12 or 14

            -- Nama & URL
            local itNameFrame = Instance.new("Frame", item)
            itNameFrame.Size = UDim2.new(1, -(isCompact and 40 or 50), 1, 0)
            itNameFrame.BackgroundTransparency = 1

            local itName = Instance.new("TextLabel", itNameFrame)
            itName.Size = UDim2.new(1, 0, 0, 18)
            itName.BackgroundTransparency = 1
            itName.Text = linkData.Name
            itName.TextColor3 = c.text
            itName.Font = Enum.Font.GothamBold
            itName.TextSize = isCompact and 11 or 13
            itName.TextXAlignment = Enum.TextXAlignment.Left

            local itUrl = Instance.new("TextLabel", itNameFrame)
            itUrl.Size = UDim2.new(1, 0, 0, 16)
            itUrl.Position = UDim2.new(0, 0, 0, 16)
            itUrl.BackgroundTransparency = 1
            local urlD = linkData.URL:gsub("^https?://", "")
            if #urlD > 24 then urlD = urlD:sub(1, 21) .. "..." end
            itUrl.Text = urlD
            itUrl.TextColor3 = c.textMuted
            itUrl.Font = Enum.Font.SourceSans
            itUrl.TextSize = isCompact and 8 or 10
            itUrl.TextXAlignment = Enum.TextXAlignment.Left

            -- Arrow kanan
            local itArrow = Instance.new("TextLabel", item)
            itArrow.Size = UDim2.fromOffset(isCompact and 16 or 20, isCompact and 16 or 20)
            itArrow.BackgroundTransparency = 1
            itArrow.Text = "→"
            itArrow.TextColor3 = c.textDim
            itArrow.Font = Enum.Font.SourceSansBold
            itArrow.TextSize = 14
            ...
            item.MouseEnter:Connect(function()
                ...
                TweenService:Create(itArrow, TweenInfo.new(0.15), {
            ...
            item.MouseButton1Click:Connect(function()
                ...
                -- Tutup dropdown
                DropHolder.Visible = false
                GetKeyBtn:FindFirstChild("SubDisplay").Text = "▼  Tap untuk pilih link"
                TweenService:Create(DropHolder, TweenInfo.new(0.15), {
                    Size = UDim2.new(1, 0, 0, 0)
                }):Play()
            end)
        end
    ...
    VerifyBtn.MouseButton1Click:Connect(function()
        ...
        if input == "" then
            SetStatus("⚠  Masukkan key terlebih dahulu!", c.warning)
            return
        end
    ...
    -- Klik overlay tutup dropdown (jika multi)
    ...
    -- ===== ANIMASI MASUK =====
    Main.Size = UDim2.fromOffset(W - 30, 0)
    ...
    local controller = {
        ...
        Show = function()
            ...
            TweenService:Create(Main, TweenInfo.new(0.25), {
                BackgroundTransparency = 0, Size = UDim2.fromOffset(W, H)
            }):Play()
        end,
